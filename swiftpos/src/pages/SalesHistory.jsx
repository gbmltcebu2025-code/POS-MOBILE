  ...
  return (
    <div className="flex-1 overflow-y-auto p-4 md:p-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mb-6">
        <StatCard label="Today's Revenue" value={`$${todayRevenue.toFixed(2)}`} icon={DollarSign} accent="text-primary" />
        <StatCard label="Today's Transactions" value={todaySales.length} icon={Receipt} />
        <StatCard label="Total Revenue" value={`$${totalRevenue.toFixed(2)}`} icon={TrendingUp} accent="text-primary" />
      </div>
            ...
            {topProducts.map(([name, data], i) => (
              <div key={name} className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-4 text-sm shrink-0">
                  ...
                  <span className="font-semibold text-primary">${data.revenue.toFixed(2)}</span>
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="space-y-2">
          {transactions.map((t) => (
            ...
            >
              ...
              <span className="text-lg font-bold text-primary shrink-0 ml-4">${t.total.toFixed(2)}</span>
            </div>
          ))}
        </div>
      )}
